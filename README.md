...
## 스프라이트 Order in Layer 용례
|Order|용도|
|-|-|
|-6|제일 멀리 있는 배경 이미지 (parallex scroll 0.8x)|
|-5|다음으로 멀리 있는 배경 이미지 (parallex scroll 0.9x)|
|-4|메인 배경 타일맵|
|-2|엘리베이터 이동 범위 타일맵|
|-1|엘리베이터 후면 타일맵, 보스전 선반|
|0|잡몹, 장식품, 박스|
|1|주인공, 타격 이펙트, 말풍선 배경 이미지|
|2|엘리베이터 전면 타일맵, world space 텍스트 (ex. 말풍선)|
